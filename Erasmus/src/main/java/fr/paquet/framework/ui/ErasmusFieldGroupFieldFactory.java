...
public class ErasmusFieldGroupFieldFactory extends DefaultFieldGroupFieldFactory {

	static {
		VaadinSession.getCurrent().setConverterFactory(new ErasmusConverterFactory());
	}
	...
	public <T extends Field> T createField(Class<?> type, Class<T> fieldType) {
		...
		} else if (ErasmusItem.class.isAssignableFrom(type)) {
	...
	protected TwinColSelect createListBuilderItem(Class type) {
		try {
			...
			Collection<ErasmusItem> values = new ErasmusContainer(item).findAll();
	...
	protected ComboBox createComboItem(Class<?> type) {
		Collection<ErasmusItem> values = new ErasmusContainer(type).findAll();
